...
export async function obtenerTodos(filtro = {}) {
    try {
        ...
        // 3- precio_min y precio_max arman UN SOLO objeto { $gte, $lte } sobre el campo precio,
        // en vez de pisarse entre sí o duplicarse en un $and como antes. Los dos límites son
        // inclusivos (>=/<=): si pedís precio_max=12000, un libro de $12000 exacto debe aparecer.
        if (filtro?.precio_min || filtro?.precio_max) {
            filtroMongo.precio = {};
            if (filtro.precio_min) filtroMongo.precio.$gte = parseInt(filtro.precio_min);
            if (filtro.precio_max) filtroMongo.precio.$lte = parseInt(filtro.precio_max);
        }
