...
/**
 * Schema para el login/registro con Google.
 * Acá NO validamos email ni password: el body solo trae el "credential",
 * que es el ID Token que genera el botón de Google en el frontend.
 * Ese token es una cadena firmada por Google (formato JWT) que todavía no
 * fue verificada — eso lo hace el service con google-auth-library, no Yup.
 */
export const googleAuthSchema = yup.object({
    credential: yup.string().required('Falta el credential de Google')
});
